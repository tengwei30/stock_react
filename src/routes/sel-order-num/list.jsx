...
class OrderList extends React.Component {
  ...
  getDetails( orderno ) {
    ...
    .done(( res ) => {
      Toast.hide();
     //  session.set( config.keys.USER_INFO, res.data.items[0].billno );// 订单信息
     //  session.set( config.keys.SUP_NAME, res.data.items[0].suppliername ); // 供应商名字
     //  session.set( config.keys.MESSAGE_ORDER, res.data.items[0].orderdate );// 日期
      // session.set( config.keys.USER_INFO, res.data.items[0].orderinmoney ); //订单金额
      // session.set( config.keys.USER_INFO, res.data.items[0].checkprogress );//验收程度
      this.setState({
        loading: false,
